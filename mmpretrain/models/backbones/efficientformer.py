...
class EfficientFormer(BaseBackbone):
    ...
    def __init__(
        ...
        assert (
            isinstance(self.layers, list)
            and isinstance(self.embed_dims, list)
            and isinstance(self.downsamples, list)
        )
        ...
        assert self.vit_num >= 0, (
            "'vit_num' must be an integer " "greater than or equal to 0."
        )
        assert (
            self.vit_num <= self.layers[-1]
        ...
        for i_layer in self.out_indices:
            if not self.reshape_last_feat and i_layer == 3 and self.vit_num > 0:
                ...
                layer = build_norm_layer(
                    dict(type="GN", num_groups=1), self.embed_dims[i_layer]
                )[1]
